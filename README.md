...
**Fidelity.** The compared theorem is the benchmark's formalisation of the erdosproblems.com phrasing ("at least (k−1)n/2 + 1 edges",
trees on k+1 vertices) rather than the classical phrasing ("more than (k−2)n/2 edges", trees on k vertices). Writing `t
= k + 1` for the tree order, the classical hypothesis is `|E| > (t-2)n/2` while the compared hypothesis is `|E| ≥
(t-2)n/2 + 1`. These agree when `(t-2)n` is even; when `(t-2)n` is odd the compared hypothesis is stronger by half an
edge, so the advertised theorem is formally very slightly weaker than the classical conjecture in that parity case. The
Lean proof's internal counting lemma derives `2|E(G)| ≤ (t-2)n` whenever the tree is absent, which is the sharp
classical bound, but only the advertised statement is compared. The vertex type is `Fin n`, which loses no generality;
`G.edgeSet.ncard` is the number of edges; `T.IsTree` and `T.IsContained G` are Mathlib's notions. The hypothesis `k + 1
≤ n` matches `n ≥ k + 1`.
...
**Statement.** The problem had no statement in Formal Conjectures. The statement was produced by Epoch AI's autoformalization pipeline and reviewed for faithfulness by Thomas F. Bloom; the file the model received is [`apn/data/erdos_autoformalized/Isolated/Erdos548.erdos_548.lean`](https://github.com/epoch-research/LeanOpenProblems/blob/77882c437ca1dfefab3b27fa00f1d29788100311/apn/data/erdos_autoformalized/Isolated/Erdos548.erdos_548.lean).
...
|---|---|---|---|---|---|
| `Erdos548/Resolutions/Erdos548_192usd_21h.lean` | **primary** (wired to `Solution.lean`) | ReAct agent, larger budget, 26 Aug 2026 | $192 | 21 h | [transcript](https://viewer.hawk.hawkbench.com/permalink/sample/DtNfGbQAp2VSeoBwkKV8bE) |
...
from the FrontierMath Erdős benchmark (LeanOpenProblems, Epoch AI, MIT licence; see `NOTICE`). Cited papers,
